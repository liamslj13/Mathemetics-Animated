...
class TakagiFunction(Scene):
    ...
    def construct(self):
        ...
        h_value = Integer(h_tracker.get_value()).next_to(n_text, RIGHT)
        h_value.scale(1.1)

        def update_h_value(h_value):
            h_value.set_value(int(h_tracker.get_value()))
        ...
        while h < 11:
            takagi_function_s = axes.plot(lambda x: sum(s(2**k * x) / 2**k for k in range(h)), color=YELLOW)
            if h == 10:
                ...
                self.play(h_tracker.animate.increment_value(1))
            h += 1
        ...
        bye_bye = Text("Bye for now!", 
                  t2w={'Bye for now!':BOLD}, 
                  t2s={'Bye for now!':ITALIC}, 
                  t2c={'Bye for now!':PURPLE}, 
                  disable_ligatures=True
        )     
